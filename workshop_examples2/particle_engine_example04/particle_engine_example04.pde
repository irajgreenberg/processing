...
void setup(){
  ...
  for (int i=0; i<colliderCount; i++){
    colliders[i] = new Collider(new PVector(random(width/2-200, width/2+200), random(height/2-200, height/2+200)), random(3, 10), #323332, false);
  }
  float theta = 0;
  float px = 0, py = 0;
  // instantiate emitters
   for (int i=0; i<emitterCount; i++){
    ...
    for (int j=0; j<particleCount; j++){
      particles[i][j] = new Particle(random(1), color(max(0, 255-(20*i)), max(0, random(80, 150)-(40*i)), min(10+(20*i), 140), 255), 20000, .85);
    }
